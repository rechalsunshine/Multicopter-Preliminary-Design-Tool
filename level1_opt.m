...
global Ph FT Dp v m_pl P_pl Cstr C MP cx Shape Mat DesignParameters DsPm MPr ...
    ...
    prop_pr_DOE   err_flag output err_percent result t RUN
...
for n=1:length(opt_target)
    ...
    lb_l1=[min(N_r)/2,min(N_c),min(Mat_body),min(d_p),Cstr(16),Cstr(14)];    % N_r=x1*2, N_c=x2*2,d_p=13+2*(x3-1), DL, TrW
    ub_l1=[max(N_r)/2,max(N_c), max(Mat_body),max(d_p),Cstr(17),Cstr(15)];   % lower boundary of variables
    ...
    [depm_opt(n,:),fval,exitflag] =...
        ga(fun,6,[],[],[],[],lb_l1,ub_l1,nonlcon_l1,intcon,options_l1);
...
%                     DL1=depm_opt(5);
    result_opt(n,:)=level2_opt(input1);
