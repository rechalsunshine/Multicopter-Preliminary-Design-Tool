function err_percent=level2_style(DL)
%timewo=tic;
global Ph FT v Cstr C m_pl P_pl MidPara DsPm Load err_flag missionerr RUN result_style FUNoutput
err_flag=[0,0,0,0,0,0,0,0,0];     % clear the cache
missionerr=[0,0,0];
err_percent=0;
FUNoutput(1:60)=result_style(1,:);                    
...
TrW=FUNoutput(8)*FUNoutput(17)/GTOM0/1000;      % Thrust weight ratio
DsPm(7)=TrW;
...
m_p=FUNoutput(29)/1000;
pr_p=FUNoutput(36);     %% call a level 3 funciton to calculate the mass and price of a single propeller
%% central body estimation
 [m_body,pr_body,A_c]=level3_body(L_d);  % mass, price, and the central frame area
m_body=FUNoutput(32)/1000;
 pr_body=FUNoutput(39);  % mass, price, and the central frame area
...
Ps_in_max=FUNoutput(41);
m_mot=FUNoutput(27)/1000;
pr_mot=FUNoutput(34);     %% call a level 3 funciton to calculate the mass and price of a single propeller
...
m_esc=FUNoutput(28)/1000;
pr_esc=FUNoutput(35);     %% call a level 3 funciton to calculate the mass and price of a single propeller
...
% end
m_arm=FUNoutput(30)/1000;
pr_arm=FUNoutput(37);     %% call a level 3 funciton to calculate the mass and price of a single propeller

%% final add up
M_tot=DsPm(1)*(m_mot+m_esc+m_p)+DsPm(6)*m_arm+m_b+m_body+m_av;
...
% FUNoutput=[Cstr(18),DsPm(1:3),DsPm(4)*1000,DsPm(5:7),DL,opt_geo(1),...
